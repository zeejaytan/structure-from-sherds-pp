...
double GlobalProblem::computeObjective() {
    // 🚀 FULL PHYSICS OPTIMIZATION SYSTEM
    //
    // EVOLUTION HISTORY:
    ...
    //   - Layer 2: Collision detection, curvature continuity, SE(3) consistency
    //   - Layer 3: Multi-constraint weighted scoring and filtering

    std::cout << "\n🚀 FULL PHYSICS OPTIMIZATION: Comprehensive constraint system active" << std::endl;

    // Use the full physics optimizer for maximum precision while preserving recall
    FullPhysicsOptimizer optimizer;
    std::vector<int> selected_hypotheses = optimizer.optimizeWithFullPhysics(*this);

    // 🔧 **CRITICAL FIX**: Apply physics filtering to actual problem structure
    // Previous version: Only computed score, didn't modify assembly hypotheses (BROKEN)
    // Fixed version: Update hypotheses vector to contain ONLY physics-approved connections
    std::cout << "🔧 PHYSICS ENFORCEMENT: Updating problem structure with filtered hypotheses" << std::endl;
    std::cout << "   Before: " << hypotheses.size() << " hypotheses" << std::endl;

    // Create new hypotheses vector with only physics-approved connections
    std::vector<ConnectionHypothesis> filtered_hypotheses;
    for (int idx : selected_hypotheses) {
        filtered_hypotheses.push_back(hypotheses[idx]);
    }

    // Replace the original hypotheses with physics-filtered ones
    hypotheses = filtered_hypotheses;
    std::cout << "   After: " << hypotheses.size() << " hypotheses (physics filtering applied)" << std::endl;

    // Compute objective based on selected hypotheses
    double total_score = 0.0;
    for (const auto& hyp : hypotheses) {  // Now using filtered hypotheses
        total_score += static_cast<double>(hyp.inlier_count) + hyp.local_score;
    }

    std::cout << "🎯 Full physics objective: " << total_score
             << " (from " << hypotheses.size() << " physics-approved hypotheses)" << std::endl;
...
void MultiHypothesisOptimizer::pruneByGlobalCompatibility(GlobalProblem& problem) {
    ...
    cout << "*** GLOBAL PRUNING *** Checking cycle consistency and collisions (hypotheses="
         << problem.hypotheses.size() << ")" << endl;

    std::vector<bool> keep_hypothesis(problem.hypotheses.size(), true);
    int pruned_count = 0;
    int axis_pruned_count = 0;

    // Check each hypothesis for global compatibility
    for (int i = 0; i < problem.hypotheses.size(); i++) {
        const auto& hyp = problem.hypotheses[i];

        // POTTERY AXIS ALIGNMENT CHECK: Reject inverted pieces
        if (checkAxisAlignment(hyp.piece_a, hyp.piece_b)) {
            // Axis alignment failed - pieces facing opposite directions
            keep_hypothesis[i] = false;
            pruned_count++;
            axis_pruned_count++;
            cout << "  PRUNED axis alignment: Pieces " << hyp.piece_a << "-" << hyp.piece_b
                 << " (hyp " << i << ", " << hyp.inlier_count << " inliers)" << endl;
            continue;
        }
    ...
    cout << "*** PRUNING COMPLETE *** Removed " << pruned_count
         << " hypotheses (" << axis_pruned_count << " axis, "
         << (pruned_count - axis_pruned_count) << " other), "
         << problem.hypotheses.size() << " remaining" << endl;
}
...
bool MultiHypothesisOptimizer::checkAxisAlignment(int piece_a_id, int piece_b_id) const {
    // POTTERY AXIS ALIGNMENT CHECK: Reject pieces with inverted/opposite axes
    // Returns true if axis check FAILED (should be pruned)

    if (!shard_data_ || piece_a_id < 1 || piece_b_id < 1 ||
        piece_a_id > shard_data_->size() || piece_b_id > shard_data_->size()) {
        return false;  // No data or invalid IDs - skip check
    }

    // Access piece geometry (piece IDs are 1-indexed)
    const auto& geom_a = (*shard_data_)[piece_a_id - 1];
    const auto& geom_b = (*shard_data_)[piece_b_id - 1];

    // Check if axis data is loaded
    if (geom_a.edge_line_.axis_norm_.size() == 0 ||
        geom_b.edge_line_.axis_norm_.size() == 0) {
        return false;  // No axis data - skip check
    }
    ...
    if (!aligned) {
        cout << "*** AXIS ALIGNMENT *** REJECTED Pieces " << piece_a_id << "-" << piece_b_id
             << " axis_alignment=" << axis_alignment
             << " (threshold=0.8, POSITIVE required - inverted axes rejected)" << endl;
        return true;  // FAILED - should be pruned
    }

    cout << "*** AXIS ALIGNMENT *** PASSED Pieces " << piece_a_id << "-" << piece_b_id
         << " axis_alignment=" << axis_alignment << endl;
    return false;  // PASSED - keep hypothesis
}
