...
void Visualize::AddMesh(view& viewer, double r, double g, double b)
{
	double value = 0;
	...
	if (value == 0) {
		...
		viewer->setPointCloudRenderingProperties(pcl::visualization::PCL_VISUALIZER_COLOR, r, g, b, mesh_name_);
	}
	...
	const vector<Trans>& T_axis)
{
	...
	cout << "Result : " << rank << "/" << num_total << ", Score : "
...
void VisCurrentState(
	...
	LCSIndex& lcs)
{
	int index = lcs.trans_.index_ - 1;
	int toward = lcs.trans_.toward_ - 1;
	Matrix3d R;
	Vector3d t;

	lcs.trans_.Output(R, t);
	shard[index].Move(R, t);

	pc[index].UpdateData(viewer, shard[index].edge_line_.point_, shard[index].edge_line_.normal_, 100, 255, 0);
	pc[toward].UpdateData(viewer, shard[toward].edge_line_.point_, shard[toward].edge_line_.normal_, 0, 100, 255);
	pc[index].AddPointCloud(viewer);
	pc[toward].AddPointCloud(viewer);
	pc[index].AddNormal(viewer);
	pc[toward].AddNormal(viewer);
}
	...
	LCSIndex& lcs)
{
	...
	lcs.trans_.InvOut(R, t);
	//breakline_move(shard[index].m_sBreakLine, R, t);
	shard[index].Move(R, t);
}
void TurnoffandReturn(pcl::visualization::PCLVisualizer::Ptr& viewer,
	vector<Visualize>& pc,
	...
	for (int i = 0; i < shard_num; i++) {
		if (manager.out_state_[index].true_node_[i]) {
			if (right_sherd[i])
				...
				pc[i].AddMesh(viewer, 0.5, 0.0, 0.0);
		}
	}
}
	...
	vector<Trans>& T_fine)
{
	...
	for (int i = 0; i < num_shard; i++) {
		for (int j = 0; j < num_graph; j++) {
			if (manager.out_state_[index].graph_[j].node_[i]) {
				Matrix3d R_i_f, R_i_g, R_i, R;
				Vector3d t_i_f, t_i_g, t_i, t;
				T_fine[i].Output(R, t);
				manager.out_state_[index].graph_[j].T_[i].InvOut(R_i_g, t_i_g);
				t[1] = t[1] + 200 * j;
				R_i_f = R.inverse();
				t_i_f = -R_i_f * t;
				R_i = R_i_g * R_i_f;
				t_i = R_i_g * t_i_f + t_i_g;
				manager.shard_[i].Move(R_i, t_i);
				pc[i].MeshTransform(R_i, t_i, viewer);
